...
- **The card list is `unique=prints`, and a Printings filter decides what shows** (One per card · All · Base · Special). `unique=cards` hid two of Gleaming Splendor's three printings in The Hobbit. `isSpecialPrinting()` is validated against that set's 321 printings and must not be loosened: `frame_effects` alone is **not** a special marker — `legendary` (116 prints) and `enchantment` (33) describe the card's type, and the *base* Gleaming Splendor carries `enchantment`. The markers are `showcase`/`extendedart`/`inverted`/`etched`, a non-ordinary `border_color`, `full_art`, `promo`, `variation`. The rule leaves exactly one base printing for 192 of 193 cards; the exception (Smaug the Magnificent, two ordinary printings) is why "base" is a filter and **not** a way to pick one printing per card — that is the lowest collector number.
- **Prices come from our own `card_prices`, one query per set code**, folded into the Scryfall-shaped `prices` object so `getPrice`/`formatPrice` and the user's `price_source` work unchanged. Not from the Scryfall payload (`slimSpoilerCard` drops it): a card must not show one number here and another in the collection. Coverage is effectively a *released-set* feature — measured 2026-09-09, The Hobbit 321/321 priced vs Star Trek 9/135 — so an unpriced spoiler is normal. Price sorts sink unpriced cards to the bottom of **both** directions; treating absent as 0 buries the genuinely cheap cards on "low to high".
- **A tile that opens a hover preview must not carry `title`.** The native tooltip fires on the same hover and lands on top of the image; use `aria-label`. Same bug and same fix as 6a8be86 in the build assistant.
...
- **A mechanic is "new" when nothing printed it before this set**, measured with `keyword:"X" -e:<code> date<<release>` per mechanic (cached 7 days, per user in `localStorage`). A frozen list of known keywords was rejected: it ages, and last year's mechanics would read as new forever.
- **Universes Beyond sets put flavor ability names in Scryfall's `keywords`.** Final Fantasy reports 108 of them ("Blizzaga", "Selfie Shot", "GOOOOAAAALLL!"), 67 postdating the established list, against 1 for Star Trek. They are genuinely new, so no list can filter them — `MAX_MECHANIC_LOOKUPS = 12` is what bounds them, and candidates are ordered by in-set card count so the budget goes to the likeliest real mechanics (a set mechanic is on many cards; a flavor name on two or three). Do not raise the cap without re-checking a UB set.
- **Only keywords that could be new are looked up** (`src/lib/establishedKeywords.js`). One search per keyword × 25-40 keywords is what trips Scryfall's rate limit, and almost none of them can change the answer: a keyword printed before the set cannot be new to it. The list is 522 keywords first printed before `ESTABLISHED_BEFORE` (2024-01-01), generated from our own `card_prints` — it cuts Star Trek from 25 lookups to 2. Going stale costs an extra lookup, never a wrong answer, because a keyword only ever moves from new to established; a list of *known* keywords used to decide novelty directly has the opposite failure and is still rejected.
- **A rate-limited Scryfall reads as a CORS/network error, not a 429.** The error response carries no CORS headers, so the browser refuses it and `runScryfallRequest`'s retry loop treats it as transient — one refused lookup becomes four, each extending the cooldown. Novelty lookups therefore pass `retries: 0` to `sfGetOrStatus` and the loop **stops at the first unanswered lookup** rather than walking the remaining keywords into the same wall.
- **A failed lookup is not a zero.** `/cards/search` answers **404** when nothing matches — a real answer, and the one that means "new" — while a 429/5xx/offline is *no answer*. `sfGet` returns null for both, so this path uses `sfGetOrStatus` and returns null on anything but a 404; null is left unflagged and **uncached**, so it retries. Reading a failure as zero labelled Ward new on The Hobbit (321 cards → enough lookups to hit the rate limit) and cached that for a week. For the same reason the lookups run **one at a time**, never `Promise.all`.
...
| `scripts/lib/scanner-harness-core.mjs` | Shared harness plumbing: Scryfall fetch/image cache, seeded RNG, the capture-degradation model, and the scenario lists both newer harnesses and the preview import |
...
**Multi-frame fusion** (`hashFusion.js`, shipped): when no sampled frame passes the acceptance gates, CardScanner per-bit majority-votes the frames' primary hash sets (art/color/full/tiles) and tries one fused match labeled `fused` — glare flips different bits in different frames, so the majority hash is cleaner than any single capture. Rescue-only: it runs after the normal path failed and goes through `shouldAcceptMatch` like any other frame, so `MATCH_ACCEPT_CEILING` binds it too. It inlined its own gate until 2026-08-08 and could accept up to `MATCH_THRESHOLD` (122) — straight through the band where every logged match has been wrong.
...
**Per-channel RGB hashes (measured 2026-08-08, NOT shipped):** external MTG sorters commonly hash R/G/B separately where we hash saturation (`phash_hex2`). `scripts/scanner-color-harness.js` (220 probes, 1050-card pool, 10 scenarios, 6 weighting configs) measured it: pooled lookalike margin p10 **57.06 → 58.04** at best, of which +0.35 is recalibration alone (`FULL_SCALE` re-measures to 1.168 on the larger pool, and `base-cal` beats `base` in all ten scenarios). Three extra 256-bit hashes per row is **+11.5 MB on a ~15 MB pack** plus a format bump and a full `--reseed` — nowhere near justified for ~1.7%, and the winning config was unstable across runs.

The one real effect: RGB is markedly more **white-balance robust** (margin p10 51.71 → 56.23 on colour-cast scenarios, ~+9%), because `computeHashFromGray` runs `percentileCap` → CLAHE *per plane* and largely normalises a per-channel gain away, whereas saturation is computed as `(max−min)/max` from raw RGB before any normalisation. On every non-cast degradation RGB is equal or slightly worse. Revisit only if colour cast becomes a real-world complaint.

**Adaptive z-score acceptance (measured 2026-08-08, NOT adopted):** tmikonen's `magic_card_detector` accepts when `(mean_of_others − min_distance) / std > 4.0` instead of on absolute thresholds. `scripts/scanner-gate-harness.js` probes cards held **out** of the reference pool alongside in-pool ones, because a looser gate always wins when every probe has a correct answer. Result: `z-all@5` matches `fixed-prod` on true-accept (99.8%) with ~19× fewer false accepts (1.15% → 0.06%) — but it needs mean/std over *every* row, which is exactly what the LSH band index exists to avoid. The affordable version (`z-top50`, stats over a truncated near-neighbour set) collapses to **84.7%** true-accept. That kills it.
