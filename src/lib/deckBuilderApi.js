...
export async function fetchPaperPrintings(name) {
  return fetchPrintingsByName(name, { language: 'all' })
}

// Compatibility exports used by version pickers and import flows.
export function pickAutomaticDeckPrinting(printings, fallbackCard = null) {
  return selectPreferredDeckPrinting({ printings, fallbackCard })?.sfCard || null
}
