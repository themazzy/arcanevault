...
const CHANGELOG = [
  {
    ...
    label: 'Latest',
    updates: [
      'Collection folder and deck membership now loads much faster with IndexedDB-first rendering plus incremental folder sync',
      'Collection ownership is enforced: a card only stays in collection while it still lives in at least one binder or collection deck',
      'Per-folder quantity badges in collection are now correct when the same card exists in multiple binders or decks',
      'Bulk selection now counts selected copies, confirms deletes, and deselects properly when quantity is reduced from 1 of N',
      'Collection filters now include binder or deck name matching',
      'Wishlists now default to grid view, use the shared binder-style card grid, and only allow bulk move to other wishlists',
      'Life Tracker fullscreen landscape was tightened for short phone screens such as the Samsung S24',
    ],
  },
  {
    version: '2026-03-31',
    label: 'Previous',
    updates: [
      'Life Tracker: unified game log with all events in one timeline from the game menu',
      'Life Tracker: MTG-themed death messages now appear when a player is eliminated',
      'Life Tracker: low-life animations pulse at 10 life and intensify at 5 life',
      'Life Tracker: counter tabs now show icon and name for clearer poison, energy, experience, and radiation tracking',
      'Light theme readability and Life Tracker button borders were fixed across pale surfaces',
      'Android back-button handling now navigates in-app and requires a double-tap to exit from home',
    ],
  },
  {
    version: '2026-03-31',
    label: 'Earlier',
    updates: [
      'Card Scanner can identify MTG cards from camera input using perceptual hashing',
      'Scanner tab added to the main navigation',
      'On-device card hash database now syncs locally for offline scanning support',
    ],
  },
  {
    version: '2026-03-26',
    label: 'Older',
    updates: [
      'Seat selection lets you swap player positions before the game starts',
      'Host setup for shared lobbies now captures name, colour, deck, and art before others join',
      'Deck win-loss stats are saved after games and shown in player config',
      'Preferred nickname in Settings auto-fills when creating a game lobby',
      'Local lobby deck selection is shown only for the host slot',
      'Bug report and feature request entry added to the app menu',
      'OLED black mode was fully fixed across surfaces',
    ],
  },
  {
    version: 'WIP',
