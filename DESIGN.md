...
| 3 | Hand-rolled modal overlays outside `UI.module.css` | **17 → 7.** Migrated the genuine centered dialogs to `Modal`/`ConfirmModal`: MoveOwnedCardsModal, SyncModal, MakeDeckModal, DeckImportModal, 2× confirmOverlay, AddCard discard, ExportModal, DeckView card detail, Home card modal (+2 dead backdrops deleted). **Fixed a real nesting bug:** two open Modals both put a capture-phase keydown on `document`, so Escape hit the OUTER one first (registration order); Modal now keeps a stack so only the topmost reacts. The **remaining 7 are intentionally NOT modals**: `Layout.mobileOverlay` (animated nav scrim), `CardComponents.filterBackdrop` (bottom sheet), `BuildAssistant.previewLightbox` + `Admin.screenshotOverlay` (image lightboxes), `DeckGoldfish.libraryOverlay` (playtest zone), `SetupWizard.overlay` (full-screen wizard), `DeckStats.previewOverlay` (z-index 9999, opens from surfaces that may already be in a modal — Modal's 200 would sit behind) |
