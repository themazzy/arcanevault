...
| 3 | Hand-rolled modal overlays outside `UI.module.css` | **17 → 9.** Migrated to `Modal`/`ConfirmModal`: MoveOwnedCardsModal, SyncModal, MakeDeckModal(fix), 2× confirmOverlay, AddCard discard, ExportModal, DeckView card detail, Home card modal — plus 2 dead backdrops deleted. **Fixed a real nesting bug in the process:** two open Modals both put a capture-phase keydown on `document`, so Escape hit the OUTER one first (registration order) and closed the wrong dialog. Modal now keeps a stack; only the topmost reacts to keys. Remaining 9 are menu backdrops / lightboxes / image previews that may not warrant a full Modal |
