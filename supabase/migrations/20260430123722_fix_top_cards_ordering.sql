...
begin
  ...
  -- top cards (gated) — deduplicate by scryfall_id FIRST, then sort by price, then limit
  if coalesce(v_show_crown, false) then

    -- shared CTE: one row per unique scryfall_id, best price variant
    with deduped as (
      ...
      left join lateral (
        select price_regular_eur, price_foil_eur from card_prices
        where set_code         = c.set_code
          and collector_number = c.collector_number
          and snapshot_date in (current_date, current_date - 1)
        order by snapshot_date desc limit 1
      ) cp on true
      where c.user_id = v_user_id
      -- DISTINCT ON picks best-priced variant per scryfall_id
    ...
    -- top 5: sort deduped results by price, take 5
    select jsonb_agg(
      jsonb_build_object(
        'name',      name,
        'set_code',  set_code,
    ...
    -- sort by price AFTER dedup, THEN limit
    order by unit_price desc nulls last
    limit 5;
