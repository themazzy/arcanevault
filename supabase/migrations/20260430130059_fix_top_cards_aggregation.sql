...
begin
  ...
  if coalesce(v_show_crown, false) then
    ...
    -- reusable dedup: one row per scryfall_id, best price variant chosen
    -- crown: single most valuable
    ...
    -- top 5 showcase: sort deduped results by price then aggregate
    select jsonb_agg(
      jsonb_build_object(
        'name',      t.name,
        'set_code',  t.set_code,
    ...
    from (
      -- step 1: dedup by scryfall_id, pick best-priced variant
      select distinct on (c.scryfall_id)
    ...
    -- step 2: sort by price, take top 5 BEFORE aggregating
    order by all_deduped.unit_price desc nulls last
    limit 5;
