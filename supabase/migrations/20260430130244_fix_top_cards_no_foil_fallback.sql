  ...
  if coalesce(v_show_value, false) then
    select coalesce(sum(
      c.qty * case when c.foil
        then coalesce(cp.price_foil_eur, 0)
        else coalesce(cp.price_regular_eur, 0)
      end
    ), 0) into v_value
    from cards c
    left join lateral (
  ...
  if coalesce(v_show_crown, false) then
    ...
    from (
      select distinct on (c.scryfall_id)
        ...
        -- no cross-variant fallback: non-foil uses regular price only, foil uses foil price only
        case when c.foil
          then coalesce(cp.price_foil_eur, 0)
          else coalesce(cp.price_regular_eur, 0)
        end as unit_price
      ...
      order by c.scryfall_id, (
        case when c.foil then coalesce(cp.price_foil_eur, 0)
                         else coalesce(cp.price_regular_eur, 0) end
      ) desc
    ) d
