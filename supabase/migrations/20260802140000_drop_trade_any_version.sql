-- Drop the per-card "any version" trade flag.
--
-- It never made sense: the For Trade binder holds specific owned rows — an exact
-- print, finish, language and condition out of the user's collection. "I'd trade
-- any printing of this" is a statement about a card the user does NOT
-- necessarily own, so hanging it off a concrete placement conflated the two.
-- `trade_note` stays — condition/language remarks are genuinely per-copy.

-- Recreate get_trade_post without any_version first, so nothing references the
-- column when it goes.
...
alter table public.folder_cards drop column if exists trade_any_version;
