-- Per-card trade options for the "For Trade" binder (the Trade Post haves):
--   trade_any_version — owner will trade any printing, not just the listed one
--   trade_note        — optional free note shown on the public post
-- These live on folder_cards; they default to false/null and are unused by every
-- other binder/list. get_trade_post surfaces them on each have.
alter table public.folder_cards
  add column if not exists trade_any_version boolean not null default false,
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
           ...
           'any_version', fc.trade_any_version, 'note', fc.trade_note,
