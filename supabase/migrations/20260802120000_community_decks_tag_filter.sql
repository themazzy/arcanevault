-- Tag filtering for the community deck browser.
--
-- My Decks could already filter by tag (client-side, over the loaded list); the
-- Deck Browser could not, because its list is paged server-side and a
-- client-side tag filter would only ever see the current page.
--
-- Adds:
--   p_tags      text[]  — decks must carry EVERY tag passed (AND), mirroring
--                         tagMode 'all' in src/lib/deckIndexFilters.js. The
--                         client sends a single tag for the 'any' case.
--   'tags' key in the result — the distinct tag vocabulary across all public
--                         decks, so the filter menu has something to list.
--                         Computed from the unfiltered base so a selected tag
--                         never disappears from the menu once picked.

drop function if exists public.get_community_decks(text, text, text[], text, int, text, int, int);
