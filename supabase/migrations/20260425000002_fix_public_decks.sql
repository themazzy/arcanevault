-- Fix get_public_decks: deduplicate linked pairs and use correct card count per deck type.
-- For a linked builder+collection pair, only the builder_deck is shown.
-- Collection decks count cards from deck_allocations; builder decks from deck_cards.
...
begin
  ...
  where f.user_id = p_user_id
    ...
    -- exclude collection decks that are paired with a builder deck (avoid duplicates)
    and not (
      f.type = 'deck'
...
-- Fix the deck count in get_public_profile with the same deduplication logic.
...
begin
  ...
  -- Count deduplicated decks (linked collection decks excluded)
