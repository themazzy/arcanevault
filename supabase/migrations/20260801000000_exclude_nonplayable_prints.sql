-- Art-series cards, tokens and emblems share card_prints/oracle_cards with real
-- printings, and they collide with real card names in two ways:
--
--   1. Art Series double-faced rows are named "<Card> // <Card>" (e.g. "Birgi,
--      God of Storytelling // Birgi, God of Storytelling") and carry the front
--      face in oracle_cards.face_names. A front-face request therefore matched
--      BOTH the real DFC and the art row, and the canonical tie-break — plain
--      `order by oc.name` — resolved alphabetically, so "Birgi // Birgi" beat
--      "Birgi // Harnfel". Every consumer then looked up prints under the art
--      card's name and found only the art card: no price, art-card image.
--
--   2. Single-faced art cards use the card's exact name (99 names currently
--      collide), so they sit in the print pool for a real card and can be
--      selected outright when nothing else is priced.
--
-- Type line is an exact discriminator here: every 'Card' / 'Card // Card' row in
-- the catalogue belongs to an Art Series set, and no real card's type line
-- begins with "Token" or "Emblem".

-- Null-safe on purpose: ~158 card_prints rows have no type_line, and an unknown
-- type is not evidence of an art card — those must keep resolving normally.
create or replace function public.is_nonplayable_print_type(p_type_line text)
returns boolean
language sql
immutable
parallel safe
...
as $$
  select coalesce(
    p_type_line in ('Card', 'Card // Card')
    or p_type_line like 'Token%'
    or p_type_line like 'Emblem%',
    false
  )
$$;

grant execute on function public.is_nonplayable_print_type(text) to anon, authenticated;

-- ── Display printings (Build Assistant tiles, buy list) ──────────────────────
...
as $function$
  ...
  resolved as (
    ...
    left join lateral (
      ...
      -- Playable rows win before the alphabetical fallback: without this an Art
      -- Series "X // X" row outsorts the real "X // Y" card on a front-face
      -- request. Kept as a preference rather than a filter so a deliberate
      -- lookup of an art card by its own name still resolves to something.
      order by
  ...
  join lateral (
    ...
    where cp.lang = 'en'
      ...
      -- Single-faced art cards carry the card's exact name, so the canonical
      -- fix above doesn't reach them; they must be excluded here too.
...
-- ── Recommendation metadata (assistant/EDHREC enrichment) ────────────────────
