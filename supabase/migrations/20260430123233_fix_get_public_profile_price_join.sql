...
begin
  ...
  -- recently added (last 8 distinct prints)
  select jsonb_agg(
    jsonb_build_object('name', sub.name, 'image_uri', cp2.image_uri)
  )
  into v_recent_cards
  from (
  ...
  -- collection value (gated; join by set_code+collector_number like the client)
  if coalesce(v_show_value, false) then
    ...
    from cards c
    left join lateral (
      select price_regular_eur, price_foil_eur
      from card_prices cp
      where cp.set_code         = c.set_code
        and cp.collector_number = c.collector_number
        and cp.snapshot_date in (current_date, current_date - 1)
  ...
  -- top cards (gated; deduplicated by scryfall_id, matched by set_code+collector_number)
  if coalesce(v_show_crown, false) then
    -- top 1 for crown block
    select jsonb_build_object(
      'name',             best.name,
      'set_code',         best.set_code,
      'collector_number', best.collector_number,
      'image_uri',        best.image_uri,
      'foil',             best.foil,
    ...
    from (
      ...
      where c.user_id = v_user_id
      order by c.scryfall_id, unit_price desc
    ) best
    order by best.unit_price desc nulls last
    limit 1;

    -- top 5 for showcase block
    select jsonb_agg(row_data order by row_data->>'price' desc)
    into v_top_cards
    from (
      select distinct on (c.scryfall_id)
        jsonb_build_object(
          ...
          'foil',      c.foil,
          'price',     case when c.foil
                         then coalesce(cp.price_foil_eur, cp.price_regular_eur)
